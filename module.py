...
def ConvDiscriminator(input_shape=(32, 32, 3),
    ...
    # 0
    h = inputs = keras.Input(shape=tf.keras.backend.prod(input_shape))

    # uncompress
    h = keras.layers.Dense(tf.keras.backend.prod((None, 32, 32, 3)))
    h = tf.reshape(h, (None, 32, 32, 3)) # unflatten
